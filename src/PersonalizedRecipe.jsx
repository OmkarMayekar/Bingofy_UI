...
class PersonalizedRecipe extends Component{
    ...
    async componentDidMount(){
        ...
        console.log("jwtToken in componentDidMount ====> ",typeof jwtToken);
            ...
            else
            {
                ...
                cleanedUsername = JSON.parse(username); 
    ...
    async buildGeneralRecipeArray()
    {
      ...
      await this.state.arrayOfRecipeResult.map(async item => (item.sharing_user_email.includes(cleanedUsername+"@google.com") && item.recipe_type == "PERSONALIZED" ? localArray.push(item) : null));
      this.setState({generalRecipeArray : localArray});
      console.log("localArray ====> ",this.state.generalRecipeArray);
    }
    ...
    render() 
    {
      ...
      { 
        return(
               <div>
                <h3 style={{fontFamily : 'fantasy', color : 'lightslategray', marginTop : '2%'}}>Personalized Recipes</h3>
